...
\large A research report and verification index}
\author{Uwe\\Independent research project}
\date{20 August 2026}
...
\begin{abstract}
The existence of a $3\times3$ magic square whose nine entries are pairwise
distinct positive integer squares remains open.  We study the prime support of
...
\textbf{Verification and assistance disclosure.}
This project was developed with extensive AI assistance.  The mathematical
claims were repeatedly checked by independent symbolic derivations, exact
programs, and adversarial review passes, but these are not a substitute for
independent expert review.  The detailed proof archive and executable checks
are public specifically to make errors discoverable.
}}
\end{center}
...
Rational-parameter relation scan & parameter height $H\le10^6$ & 0 \\
Dominant-block identity scan & complementary block $M\le10^6$ & 0 \\
Two-block factor scan & $\max(P,Q)\le10^{12}$ & 0 \\
\bottomrule
\end{tabular}
\end{center}
The rational-parameter run enumerated 159,139 normalized values and
12,662,531,091 unordered pairs.  The center-root and factor scans use different
heights, so their zero counts are not interchangeable.  A larger rational
parameter run may be useful as evidence but cannot settle the Diophantine
question.
...
43 tests covering the finite classifications, arithmetic stages, Gaussian
...
for independent discussion.  Responsibility for any remaining errors lies
with the human maintainer of this research archive.
